    ...
    for i in range(image.size[0]): # for every pixel:
        for j in range(image.size[1]):
            ...
            if(pixel == (255, 255, 255) or pixel == (255, 255, 255, 255)):
                ...
                y1 = min(j, y1)
                y2 = max(j, y2)
    newImage = Image.new('RGB', (x2 - x1 + 1, y2 - y1 + 1))
...
def normalizeTo256(image):
    newImage = getBoundedImage(image)
    newImage = proportionalResize(newImage)
    newImage = placeOntoCentered(newImage)
    return newImage
